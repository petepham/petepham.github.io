...
##### Working Directory #####
setwd('/home/petepham/Documents/wd/School/STAT 573/Final Project/data')
...
##### Tabling #####
method = c("Normal","Normal","Poisson","Poisson","Bootstrap-t", "Bootstrap-t","Bootstrap-t, 20% TM","Bootstrap-t, 20% TM")
group = rep(c("High OP Visits", "Low OP Visits"), 4)
mean = c(
    mean.high, mean.low,
    mean.high, mean.low,
    bs.mean.high, bs.mean.low,
...
df.results = data.frame(method, group, mean, lower, upper) %>%
    mutate(
        mean = round(mean, 3),
        lower = round(lower, 3),
        upper = round(upper, 3)
...
colnames(df.results) = c("Method", "Group", "Sample Mean", "95% Lower Bound", "95% Upper Bound")
```
